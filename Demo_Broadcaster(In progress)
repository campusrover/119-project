...
# the turtle name later.
    ...
    turtlename1 = rospy.get_param('~turtle', 'guardw')
    turtlename2 = rospy.get_param('~turtle', 'guards')
    turtlename3 = rospy.get_param('~turtle', 'guarde')
    turtlename4 = rospy.get_param('~turtle', 'guardn')
    turtlename5 = rospy.get_param('~turtle', 'intruder')
    #Get a new param turtle2
    rospy.Subscriber('/%s/pose' % turtlename1,
                     turtlesim.msg.Pose,
                     handle_turtle_pose,
                     turtlename1)
    #The added subscriber create another transformation for turtle2 which is about the same with the one with turtle1. With this method, we can let three turtle move in a chain, one follow one.
    rospy.Subscriber('/%s/pose' % turtlename2,
                     turtlesim.msg.Pose,
                     handle_turtle_pose,
                     turtlename2)
    rospy.Subscriber('/%s/pose' % turtlename3,
                     turtlesim.msg.Pose,
                     handle_turtle_pose,
                     turtlename3)
    rospy.Subscriber('/%s/pose' % turtlename4,
                     turtlesim.msg.Pose,
                     handle_turtle_pose,
                     turtlename4)
    rospy.Subscriber('/%s/pose' % turtlename5,
                     turtlesim.msg.Pose,
                     handle_turtle_pose,
                     turtlename5)
    rospy.spin()
